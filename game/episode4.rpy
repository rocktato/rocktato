label ep_4:
    ...
    rt "I {w=0.2}drink {w=0.4}now..."

    show gin thankful

    gin "Okay lil' buddy, {w=0.2}what's the problem?"

    show rt mad at shake(rate=0.01,strength=5.1,loop=5)

    rt "I don't- {w=0.2}it's- "

    show rt hurt at shake(rate=0.01,strength=6.2,loop=15)

    extend"pppppppppppppppp IT'S NOT A BIG DEAL!"
    ...
    show rt mad at shake(rate=0.01,strength=7.1,loop=20)
    ...
    show gin evil at shake(rate=0.01,strength=7.2,loop=16)

    gin "Slrrppp..{w=0.5}{nw}"
    ...
    show gin evil at shake(rate=0.01,strength=7.1,loop=30)

    extend " SLSLSLLRLRPSRPSLRSPRLPPP"
    ...
    gin "Today we're gonna uhhhhhhhh...... {w=0.4}{nw}"

    show gin thankful

    extend "you wanna go fishing?"
    ...
    show rt sad at Position(xpos=0), bounce
    ...
    show gin bruh at Position(xpos=750) with ease

    no "The cowboy jumped into the water and stood there, {w=0.2}silently, {w=0.2}waiting."

    no "When they found a fish floundering about, {w=0.2}they quietly crept towards it, "

    show gin evil 2 at bounce

    extend "and STABBED it with the makeshift rod."
    ...
    show gin at Position(xpos=400)

    show rt proud at Position(xpos=750)

    with ease

    no "Rocktato stood in the shallow waters, {w=0.2}gripping the fishing rod tightly with his two baby hands."

    no "He saw a small fish just wiggling about {w=0.2}{nw}"

    show rt mad at bounce

    extend "and he JUMPED at it!"

    no "It swam away."

    show rt at shake(rate=0.01,strength=4.8,loop=2)

    rt "Nooooooo, {w=0.2}get back here you stupid- {w=0.2}uh- {w=0.2}uh... {w=0.2}fish!"

    show rt at phrog_goes_in_and_out_a_door

    no "He kept chasing the fish around, {w=0.2}but it wouldn't slow down."

    hide rt

    show rt sad at none, Position(xpos=1000)

    no "It eventually swam far out into the deeper waters."

    show gin thankful

    gin "Maybe er- {w=0.2}stop runnin' 'n jumpin' at it before jabbin' it!"
    ...
    no "Rocktato spotted another fish and, {w=0.2}instead of jumping at it, {w=0.2}{nw}"

    show rt at bounce

    extend "he cast out the line."
    ...
    show obj cod at Position(xpos=900, ypos=450), bounce
    ...
    rt "Aw... {w=0.2}Gin... {w=0.2}now I don't have the strength to eat it..."

    gin "Hey, {w=0.2}that alright, {w}{nw}"
    ...
    gin "She grabbed her whip and then unsummoned it."
    ...
    rt "I will {w=0.2}I will {w=0.2}I will!"
    ...
    show fg bikeride ground

    show obj bikeride at motorcycle_vibrate
    ...
    show bg fire apparatus

    show rt joy 2 at Position(xpos=0), bounce

    show gin evil 2 at Position(xpos=300),bounce
    ...
    no "She checks her phone and.. {w=0.4}she {w=0.2}made it just on time?"
    ...
    show gin uncomfortable

    gin "So are all the firetrucks..."

    show rt uncomfortable at bounce
    ...
    show cg 4 smoke with hpunch

    no "It was absolutely flooded with smoke!"

    no "Traces of fire could be seen coming out the equipment room."
    ...
    show cg 4 equipmentgone with hpunch
    ...
    show bg fire hallway

    show rt proud at Position(xpos=500)

    show gin sad at Position(xpos=850)
    ...
    fm "We burned all ya SHNOOT! {w=0.4} and we took all ya TRUCKS! {w=0.4}and now, {w=0.2}ya got nothin' ta fight fires with!"
    ...
    show cg 4 holdfire angry

    no "It was struggling to breathe now."

    gin "{b}WHERE ARE THEY?{/b}"

    fm "HAH!!! {w=0.5}Not... {w=0.3}like... {w=0.3}I'd tell ya."
    ...
    show rt sad 2 at Position(xpos=850)

    show gin bruh at Position(xpos=500)
    ...
    gin "Ehhy that's easy."

    gin "We put tracking devices on all our vehicles."

    gin "We'll just follow 'em {w=0.3}and that should take us straight to the other firefighters."
    ...
    no "After a completely wild ride, the two had finally found where the firetrucks were parked."

    hide fx

    show bg parkinglot night

    show rt sad at Position(xpos=200)

    show gin bruh at Position(xpos=550)
    ...
    show fireman o as fireman1 at Position(xpos=600)

    show fireman joy as fireman2 at Position(xpos=770)

    show fireman smug as fireman3 at Position(xpos=960)

    show gin bruh at Position(xpos=120) with easeinleft

    show rt sad at Position(xpos=320) with easeinleft
    ...
    show fireman joy as fireman2 at bounce
    ...
    show fireman happy as fireman2
    ...
    show fireman mad as fireman3 at bounce

    fm "I mean.. {w=0.2}there's still that stupid COWBOY, but we'll out numba 'em at that point... "

    show gin bruh

    show fireman smug as fireman3

    extend "like what are they gonna do???"

    show obj 4 hostages at bounce

    ff1 "You.. {w=0.2}you can't do this!"

    show fireman cool as fireman1

    fm "Well... {w=0.2}we can... {w}And we did..."

    ff1 ""

    ff1 "Well shoot."

    show fireman joy as fireman1 at bounce

    show fireman joy as fireman2 at bounce

    show fireman joy as fireman3 at bounce
    ...
    show gin at Position(xpos=50)

    show rt mad at Position(xpos=550)

    with ease

    show fireman shock as fireman1

    show fireman mad 2 as fireman3

    show fireman hurt as fireman2

    no "Rocktato ran into the ritual and started spraying his pocket water everywhere!"

    hide fireman2 with dissolve

    fm "AAAAAAAHHHHHHHHhhhhhhhhhhhh"
    ...
    no "Rocktato noticed they had disappeared from their hiding spot."
    ...
    show gin angey 2 at Position(xpos=450) with easeinleft
    ...
    rt "But... {w=0.2}I totally embarrassed myself at the end there... {w=0.4}GUHhhhhhhhhhhhhhhhhhhh..."
